...
namespace Gwen.Net.Tests.Components
{
    ...
    public class RichLabelTest : GUnit
    {
        ...
        private readonly Font f3;
        private readonly Font f4;
        private readonly Font f5;

        public RichLabelTest(ControlBase parent) : base(parent)
        {
            f1 = new Font(Skin.Renderer, "Arial", size: 15);

            f2 = new Font(Skin.Renderer, "Times New Roman", size: 20);
            f2.Bold = true;

            f3 = new Font(Skin.Renderer, "Courier New", size: 15);
            f3.Italic = true;

            f4 = new Font(Skin.Renderer, "Times New Roman", size: 20);
            f4.Bold = true;
            f4.Underline = true;
        ...
        {
            RichLabel label = new(parent);
            label.Dock = Dock.Fill;
            label.LinkClicked += OnLinkClicked;

            Document document = new();

            document.Paragraph().Font(f1).Text("This test uses Arial 15, Red. Padding. ", Color.Red).Font(f2)
                .Text("This text uses Times New Roman Bold 20, Green. Padding. ", Color.Green).Font(f3)
                .Text("This text uses Courier New Italic 15, Blue. Padding. ", Color.Blue).Font().Text("Test link (")
                .Link("Test Link 1", "Test Link 1", Color.Blue, new Color(value: 0xFFADD8E6)).Text("). ").Font(f2)
                .Text("Test link custom font (", new Color(value: 0xFFFF00FF)).Link(
                    "Test Link 2",
                    "Test Link 2",
                    Color.Blue,
                    new Color(value: 0xFFADD8E6),
                    f4).Text(").", new Color(value: 0xFFFF00FF));

            document.Paragraph().Font(f5).Text("\n").Text("document.Paragraph().").LineBreak().LineBreak()
                .Text("\tFont(f1).").LineBreak()
                .Text("\t\tText(\"This test uses Arial 15, Red. Padding. \", Color.Red).").LineBreak()
                .Text("\tFont(f2).").LineBreak()
                .Text("\t\tText(\"This text uses Times New Roman Bold 20, Green. Padding. \", Color.Green).")
                .LineBreak().Text("\tFont(f3).").LineBreak()
        ...
        private static Document LongDocument()
        {
            ...
            document.Paragraph(Margin.Ten, firstIndent: 20).Text
            (
            ...
            document.Image(
                ...
                Color.White,
                new Margin(left: 20, top: 10, right: 20, bottom: 10));
